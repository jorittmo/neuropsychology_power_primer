...
scenarios will be given, from the first author’s recent research.
...
size-weight illusion [@mcintosh_disrupted_2016]. This pilot study
...
discussions 20[e.g. @crawford_detecting_2006; @crawford_methods_2006; @mcintosh_simple_2018]. The
...
for deficits and dissociations. Figures \@ref(fig:FIG2) and \@ref(fig:FIG2) and
Tables \@ref(tab:TD-tab) and \@ref(tab:RSDT-tab) show power across a range of
likely parameters, and should enable ballpark estimates, adequate for most
purposes. The `singcar` package for R supports power calculations, and for a
wider range of tests (Rittmo & McIntosh, 2020). However, the ability to
calculate power for a given effect size may engender a false sense of precision
or control. In fact, the estimates of effect size in single-case studies are
themselves subject to a high degree of uncertainty, with very wide confidence
(or credible) intervals [e.g. @crawford_point_2010], and power calculations
based on such estimates will also be imprecise. Moreover, in a typical
neuropsychological case study, we have little control over the effect sizes we
encounter, and limited scope to achieve a desired level of power. Nonetheless, a
priori power calculations can play useful roles in study design and
interpretation, as illustrated in Section \@ref(section9).
...
The code for our power calculations and simulations is available 
[here](https://github.com/jorittmo/neuropsychology_power_primer).
Power calculations for a wider range of single-case tests are supported by the
singcar package for R, the developmental version of which can be accessed 
[here](https://github.com/jorittmo/singcar)^[https://github.com/jorittmo/singcar].
